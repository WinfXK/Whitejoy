...
public class FishPlayer {
    ...
    public static final File file = new File(Whitejoy.getMain().getConfigDir(), Whitejoy.SBItemFileName);
    public static final String[] BaseKey = {"{Duration}", "{GameTime}", "{Top}", "{Player}", "{Money}", "{FishCount}", "{Rank}", "{FishName}", "{Size}", "{Length}"};
    public static final Config config = new Config(file);
    ...
    protected void Start() {
        ...
        if (SBModle && Tool.getRand(0, SBRand.Max) < SBRand.Min) {
            ...
            if (item == null) return;
            item.handle(player);
            player.sendMessage(Whitejoy.getMain().getMessage().getSon("Game", "Disaster", SBFishKey, new Object[]{item.getName(player)}, player));
            event.setLoot(Item.get(0));
            return;
        }
        if (FiveModle && Tool.getRand(0, FiveRand.Max) < FiveRand.Min) {
            event.setLoot(getItem());
            player.sendMessage(Whitejoy.getMain().getMessage().getSon("Game", "Sundries", SBFishKey, new Object[]{item.getCustomName()}, player));
            return;
        }
    ...
    private static double getSize(double Length) {
    ...
    protected static Item getItem() {
        ...
        return item == null ? Item.get(349) : item.clone();
    }
